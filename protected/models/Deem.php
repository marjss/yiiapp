...
{ 
	...
	{
		return '<div class="hint">' . Deem::t($message) . '</div>'; 
	}
	...
	public static function getAvailableLanguages () {
		...
		if($languages===false) {
			$translationTable = Deem::module()->translationTable;
	...
	{
		$_SESSION['yum_message'] = Deem::t($message);
	 ...
	 * In addition to that, the message is being translated by Deem::t() */
	public static function log($message,
			$level = 'info',
			$category = 'application.modules.user.controllers.YumController') {
		if(Deem::module()->enableLogging)
			return Yii::log(Deem::t($message), $level, $category);
	}
	...
	public static function p($string, $params = array()) {
		return '<p>' . Deem::t($string, $params) . '</p>';
	}
	...
	{
		$yumBaseRoute=Deem::module()->yumBaseRoute;
		$tr=array();
		$tr['{yum}']=$yumBaseRoute;
	...
	{
		return CHtml::tag('p',array('class'=>'note'),Deem::t(
